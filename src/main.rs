...
async fn send_lobby_timeout_verdict(
    ...
    if active_lobbies_guard.get(lobby_id).unwrap().bots.len() == 10 {
        ...
        send_message_to_bots(writers, "11".as_bytes()).await;
        return;
    }
    ...
    let writers: Vec<_> = lobby.bots.values().map(|b| Arc::clone(&b.tcp_writer)).collect();
    drop(active_lobbies_guard);
    send_message_to_bots(writers, "10".as_bytes()).await;
}
...
async fn serve_for_in_game_event(
    ...
    while let Ok(in_game_event) = receivers.in_game_event.recv().await {
        ...
        send_message_to_bots(writers, payload.as_bytes()).await;
    }
...
async fn send_in_game_parameters_timeout_verdict(
    ...
    let writers: Vec<_> = lobby.bots.values().map(|b| Arc::clone(&b.tcp_writer)).collect();
    drop(active_lobbies_guard);
    send_message_to_bots(writers, "50".as_bytes()).await;
}
    ...
    while let Ok(game_aborted) = receivers.game_aborted.recv().await {
        ...
        if let Some(lobby) = removed {
            ...
            let writers: Vec<_> = lobby.bots.iter()
                ...
                .map(|(_, bot)| Arc::clone(&bot.tcp_writer))
                .collect();
            send_message_to_bots(writers, "70".as_bytes()).await;
    ...
    while let Ok(game_ended) = receivers.game_ended.recv().await {
        ...
        if let Some(lobby) = removed {
            ...
            let writers: Vec<_> = lobby.bots.iter()
                ...
                .map(|(_, bot)| Arc::clone(&bot.tcp_writer))
                .collect();
            send_message_to_bots(writers, "80".as_bytes()).await;
