...
const CHANNEL_CAP_HOT: usize = 512;
const CHANNEL_CAP_COLD: usize = 256;
...
async fn write_to_bots(
    writers: impl IntoIterator<Item = Arc<Mutex<OwnedWriteHalf>>>,
    payload: &[u8],
) {
...
async fn serve_for_lobby_found(
    ...
    while let Some(lobby_found) = lobby_found_rx.recv().await {
        ...
        async {
            let mut timer = telemetry::OpTimer::start("serve_lobby_found");
...
async fn send_lobby_timeout_verdict(
    ...
    if active_lobbies_guard.get(lobby_id).unwrap().bots.len() == 10 {
        ...
        let writers: Vec<_> = lobby.bots.values().map(|b| Arc::clone(&b.tcp_writer)).collect();
        drop(active_lobbies_guard);
        write_to_bots(writers, "11".as_bytes()).await;
        return;
    }
    ...
    let writers: Vec<_> = lobby.bots.values().map(|b| Arc::clone(&b.tcp_writer)).collect();
    drop(active_lobbies_guard);
    write_to_bots(writers, "10".as_bytes()).await;
}
...
async fn serve_for_in_game_event(
    ...
    while let Some(in_game_event) = in_game_event_rx.recv().await {
        ...
        let writers = {
            ...
            let Some(lobby) = active_lobbies_guard.get(&in_game_event.lobby_id) else {
                tracing::error!(%in_game_event.lobby_id, "Lobby not found");
                continue;
            };
            lobby.bots.iter()
                .filter(|(bot_number, _)| bot_number.as_str() != in_game_event.bot_number)
                .map(|(_, bot)| Arc::clone(&bot.tcp_writer))
                .collect::<Vec<_>>()
        };
        ...
        write_to_bots(writers, payload.as_bytes()).await;
    }
...
async fn serve_for_in_game_parameters(
    ...
    while let Some(in_game_parameters) = in_game_parameters_rx.recv().await {
        ...
        let (roles_payload, replay_payload, notify, spawn_receiver) = if lobby.in_game_parameters_fullness_controller_sender.is_some() {
            ...
            let (primary_role, roles_payload) = roles_distributor(lobby, &in_game_parameters);
            let replay_payload = starter_replays_numbers_distributor(lobby, &in_game_parameters, &primary_role);
            let notify = lobby.in_game_parameters_fullness_controller_sender.clone();
            ...
            let (primary_role, roles_payload) = roles_distributor(lobby, &in_game_parameters);
            let replay_payload = starter_replays_numbers_distributor(lobby, &in_game_parameters, &primary_role);
            (roles_payload, replay_payload, None, Some(receiver))
        };
        drop(active_lobbies_guard);

        write_to_bot(&in_game_parameters.tcp_writer, roles_payload.as_bytes()).await;
...
fn roles_distributor(lobby: &mut Lobby, in_game_parameters: &InGameParameters) -> (String, String) {
    ...
    let roles_payload = format!("2{}", roles_for_bot.join(","));
    (primary_role, roles_payload)
}

fn starter_replays_numbers_distributor(
    lobby: &mut Lobby,
    ...
    let replay_payload = format!("3{}", starter_replay_number);
    pool.remove(index);
...
async fn send_in_game_parameters_timeout_verdict(
    ...
    let mut active_lobbies_guard = active_lobbies.lock().await;
    let lobby = active_lobbies_guard.get_mut(lobby_id).unwrap();
    if lobby.in_game_parameters_requesting_fullness == 10 {
        drop(active_lobbies_guard);
        distribute_ongoing_win_team(lobby_id, active_lobbies, bots_statistics).await;
        return;
    }
    ...
    let writers: Vec<_> = lobby.bots.values().map(|b| Arc::clone(&b.tcp_writer)).collect();
    drop(active_lobbies_guard);
    write_to_bots(writers, "50".as_bytes()).await;
}
...
async fn serve_for_game_aborted(
    ...
    while let Some(game_aborted) = game_aborted_rx.recv().await {
        ...
        let removed = {
            ...
            let removed = active_lobbies_guard.remove(&game_aborted.lobby_id);
            if removed.is_some() {
        ...
        if let Some(lobby) = removed {
            ...
            let writers: Vec<_> = lobby.bots.iter()
                .filter(|(bot_number, _)| bot_number.as_str() != game_aborted.bot_number)
                .map(|(_, bot)| Arc::clone(&bot.tcp_writer))
                .collect();
            write_to_bots(writers, "70".as_bytes()).await;
...
async fn serve_for_game_ended(
    ...
    while let Some(game_ended) = game_ended_rx.recv().await {
        ...
        async {
            // Duplicate lobby report is idempotent success, not an error.
            ...
            let removed = {
                ...
                let removed = active_lobbies_guard.remove(&game_ended.lobby_id);
                if removed.is_some() {
            ...
            if let Some(lobby) = removed {
                ...
                let writers: Vec<_> = lobby.bots.iter()
                    .filter(|(bot_number, _)| bot_number.as_str() != game_ended.bot_number)
                    .map(|(_, bot)| Arc::clone(&bot.tcp_writer))
                    .collect();
                write_to_bots(writers, "80".as_bytes()).await;
